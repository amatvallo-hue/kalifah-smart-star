...
export const Route = createFileRoute("/darjah/$darjahId_/percubaan-mpt4/$subjekId_/$setId")({
  head: () => ({
    meta: [{ title: "Percubaan MPT4 — Dalam Pembinaan — Kalifah.my" }],
  }),
  ssr: false,
  component: PercubaanMpt4StubPage,
});

function PercubaanMpt4StubPage() {
  const navigate = useNavigate();
  const { darjahId, subjekId } = useParams({ from: "/darjah/$darjahId_/percubaan-mpt4/$subjekId_/$setId" });
      ...
      <main className="container mx-auto px-4 py-16 text-center">
        <div className="mx-auto max-w-md rounded-3xl border border-border/60 bg-card p-8 shadow-card">
          ...
          <h1 className="mt-4 font-display text-2xl font-extrabold text-foreground">
            Skrin jawapan sedang dibina
          </h1>
          <p className="mt-2 text-muted-foreground">
            Akan siap tidak lama lagi. Terima kasih atas kesabaran!
          </p>
          <Link
