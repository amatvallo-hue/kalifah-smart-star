...
const NOTA_TEXT: Record<string, { bm: string; en: string }> = {
  tiba_esok: { bm: "(keesokan hari)", en: "(next day)" },
  waktu_Sydney: { bm: "(waktu Sydney)", en: "(Sydney time)" },
};

export function JadualWaktu({ mula, tamat, rehatMinit, nota, aktiviti, waktuList, tempohJam, bahasa = "bm" }: Props) {
  const L = bahasa === "en"
    ? { mula: "Start", tamat: "End", rehat: "Break", minit: "minutes", esok: "(next day)", tajuk: "Schedule", tempoh: "Duration", jam: "hours" }
    : { mula: "Mula", tamat: "Tamat", rehat: "Rehat", minit: "minit", esok: "(keesokan hari)", tajuk: "Jadual Waktu", tempoh: "Tempoh", jam: "jam" };
  ...
  if (waktuList && waktuList.length > 0) {
    const rows = waktuList.length;
    const footerH = typeof tempohJam === "number" ? 30 : 0;
    const notaH = nota && NOTA_TEXT[nota] ? 22 : 0;
    const H = titleH + rows * rowH + footerH + notaH + 24;
    const col1X = padX + 8;
        ...
        {waktuList.map((w, i) => {
          ...
          return (
            <g key={i}>
              ...
              {i < waktuList.length - 1 && (
                <line x1={padX} y1={y + rowH} x2={W - padX} y2={y + rowH} stroke="#E5E7EB" strokeWidth={1} />
              )}
            </g>
          );
        })}
        {typeof tempohJam === "number" && (
          <text x={W / 2} y={titleH + rows * rowH + 20} textAnchor="middle" fontSize={13} fontWeight={700} fill={STROKE} fontFamily="sans-serif">
            {L.tempoh}: {tempohJam} {L.jam}
          </text>
        )}
        {nota && NOTA_TEXT[nota] && (
          <text x={W / 2} y={titleH + rows * rowH + footerH + 16} textAnchor="middle" fontSize={11} fontWeight={600} fill="#6B7280" fontFamily="sans-serif">
            {bahasa === "en" ? NOTA_TEXT[nota].en : NOTA_TEXT[nota].bm}
          </text>
        )}
      </svg>
    );
